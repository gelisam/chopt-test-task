...
Assume that we have a deterministic algorithm guaranteeing that all the nodes agree regardless of which messages were dropped, and consider a particular execution of the algorithm in which a fixed set of nodes exchange a number of messages and terminate with output `o`. This execution can be represented by a graph indicating causal relationships between events: each send, receive or timeout event points to the next event on the same node, and successfully delivered messages point from the sent event of the sender node to the receiver event of the receiving node. Clearly such a graph can have no cycles, or that would indicate a causality paradox. So we have a DAG.
